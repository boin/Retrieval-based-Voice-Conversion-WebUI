...
if if_gpu_ok and len(gpu_infos) > 0:
    gpu_info = "\n".join(gpu_infos)
    default_batch_size = max(min(mem) // 2, 6)
else:
...
def change_mix_choices():
    ...
    for name in os.listdir(weight_root):
        if name.endswith(".pth"):
            names.append(name.removesuffix(".pth"))
    result = {
...
def update_mix_name(
    ckpt1: str, ckpt2: str, ckpt3: str, ckpt4: str, alpha1: float, alpha2: float, alpha3: float, alpha4: float
) -> str:
    """
    根据输入参数生成新的名字字符串。

    参数:
    - ckpt1, ckpt2, ckpt3, ckpt4: 模型检查点文件的路径。
    - alpha1, alpha2, alpha3, alpha4: 用于生成名字的浮点数或整数。
...
with gr.Blocks(title="RVC WebUI") as app:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem(i18n("训练")):
            ...
            with gr.Row():
                exp_dir1 = gr.Textbox(label=i18n("输入实验名"), value="", placeholder="女声_独立_ZHY")
                sr2 = gr.Radio(
                    label=i18n("目标采样率"),
                    choices=["40k", "48k"],
                    value="48k",
        ...
        with gr.TabItem(i18n("音色融合")):
            with gr.Group():
                ...
                ckpt_a.change(update_mix_name, [ckpt_a, ckpt_b, ckpt_c, ckpt_d, alpha_a, alpha_b, alpha_c, alpha_d], name_to_save0)
                ckpt_b.change(update_mix_name, [ckpt_a, ckpt_b, ckpt_c, ckpt_d, alpha_a, alpha_b, alpha_c, alpha_d], name_to_save0)
                ckpt_c.change(update_mix_name, [ckpt_a, ckpt_b, ckpt_c, ckpt_d, alpha_a, alpha_b, alpha_c, alpha_d], name_to_save0)
                ckpt_d.change(update_mix_name, [ckpt_a, ckpt_b, ckpt_c, ckpt_d, alpha_a, alpha_b, alpha_c, alpha_d], name_to_save0)
                refresh_button.click(change_mix_choices, None, [ckpt_a, ckpt_b, ckpt_c, ckpt_d])
